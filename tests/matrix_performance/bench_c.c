...
int bench_driver(int backend,int op,int n,int m,int p,int reps,int pattern,int seed) {
 if(backend<0||backend>2||op<1||op>20||n<0||n>256||m<0||m>256||p<0||p>256||reps<1||pattern<0||pattern>1)return 2;
 ...
 if(backend==2)sink=bench_ada_baseline_run(op,n,m,p,8,pattern,a,b,u,v,d,r,ind);else if(backend==1)sink=bench_ada_run(op,n,m,p,8,pattern,a,b,u,v,d,r,ind);else sink=c_run(op,n,m,p,8,pattern,a,b,u,v,d,r,ind);
 init(a,b,u,v,d,r,ind,n,size,pattern,seed);
 ...
 double result=backend==2?bench_ada_baseline_run(op,n,m,p,reps,pattern,a,b,u,v,d,r,ind):backend==1?bench_ada_run(op,n,m,p,reps,pattern,a,b,u,v,d,r,ind):c_run(op,n,m,p,reps,pattern,a,b,u,v,d,r,ind);
