package body MJ.Matrices with SPARK_Mode is
   ...
   --  For two to five rows, avoid the setup cost of a wider loop transform.
   ...
   procedure Transpose_5_Rows (R : out Matrix; A : Matrix) with
     Inline_Always, Relaxed_Initialization => R, Global => null,
     Pre => A'Length (1) = 5 and then Valid (A) and then R'First (1) = 0 and then R'First (2) = 0 and then R'Length (1) = A'Length (2) and then R'Length (2) = A'Length (1),
     Post => (Static => R'Initialized and then (for all K in R'Range (1) => (for all L in R'Range (2) => R (K, L) = A (L, K))))
   is
   begin
      for I in R'Range (1) loop
   ...
   begin
      ...
      case A'Length (1) is
         ...
         when 5 => Transpose_5_Rows (R, A);
         when 6 .. 15 => Transpose_Pairs (R, A);
         when 16 | 64 =>
