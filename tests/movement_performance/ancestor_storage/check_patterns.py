#!/usr/bin/env python3
"""Check exhaustive ancestor patterns, lifecycle and exact packing in both build modes."""
...
shutil.copy2(HERE/'ancestor_probe.adb',src/'tests/movement_performance/ancestor_probe.adb')
...
for variant in ['checked','release']:
    work=a.out/variant;work.mkdir();project=isolated_project(src,work,'ancestor_probe')
    text=project.read_text().replace('project Fragment is','project Fragment is\n   for Main use ("ancestor_probe.adb");\n   for Exec_Dir use "bin";')
    if variant=='release':
    ...
    binary=work/'bin/ancestor_probe'
