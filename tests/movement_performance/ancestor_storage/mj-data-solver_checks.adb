...
package body MJ.Data.Solver_Checks is
   procedure Check (D : in out Simulation) is
      ...
      Last : constant Natural := (if N > 0 then AR.Length (D.Ancestors, N - 1) - 1 else 0);
   ...
   begin
      if Last < 1 or else D.Solver_Policy /= Compatible then return; end if;
      ...
      for Offset in 0 .. Last - 1 loop
         declare
            Bad : constant Natural := AR.Column (D.Ancestors, N - 1, Offset);
            Par : constant Natural := AR.Column (D.Ancestors, N - 1, Last - 1);
         begin
         ...
         if Bad = Par then
            ...
            D.Dynamics.Mass ((N - 1) * N + Par) := -1.0e-20;
            D.Dynamics.Mass (Par * N + N - 1) := -1.0e-20;
            D.Dynamics.Mass ((N - 1) * N + Bad) := Work_Limit;
            D.Dynamics.Mass (Bad * N + N - 1) := Work_Limit;
            D.Dynamics.Mass (Par * N + Bad) := Work_Limit;
            D.Dynamics.Mass (Bad * N + Par) := Work_Limit;
         end if;
         D.First_Clamped := -1;
