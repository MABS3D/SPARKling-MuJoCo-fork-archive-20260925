#!/usr/bin/env python3
"""Run disjoint packed-row lengths, untouched ranges and rejected lanes in both builds."""
import argparse
import hashlib
...
src = a.out / 'source'
for source in source_files(REPO) + [HERE / 'disjoint_rows_probe.adb']:
...
for variant in ('checked', 'release'):
    work = a.out / variant
    work.mkdir()
    project = isolated_project(src, work, 'disjoint_rows_probe')
    text = project.read_text().replace('project Fragment is',
        'project Fragment is\n   for Main use ("disjoint_rows_probe.adb");\n   for Exec_Dir use "bin";')
    if variant == 'release':
    ...
    binary = work / 'bin/disjoint_rows_probe'
