...
for variant in ['current','checked']:
    ...
    for section in ['.text','.rodata']:
        ...
        subprocess.run(['objcopy','--dump-section',section+'='+str(old),str(previous),str(work/('prior-copy'+section+'.elf'))],check=True)
        subprocess.run(['objcopy','--dump-section',section+'='+str(new),str(binary),str(work/('final-copy'+section+'.elf'))],check=True)
