#!/usr/bin/env python3
"""Build or run diagnostic instrumentation on the frozen policy build.
...
def main():
    ...
    if not a.fixtures:
        ...
        path.write_text(text)
        path=smooth/'mj-data-kinematics.adb';text=path.read_text()
        text=text.replace('      --  Jv/Jw refer', '      Movement_Profile.Start (Jacobians);\n      --  Jv/Jw refer',1)
        text=text.replace('      D.Cache.Pose_Valid := True;', '      Movement_Profile.Stop (Jacobians);\n      D.Cache.Pose_Valid := True;',1)
        path.write_text(text)
