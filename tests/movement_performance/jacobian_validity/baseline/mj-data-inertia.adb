...
package body MJ.Data.Inertia with SPARK_Mode is
   ...
   --  Dense representation of the C reverse L' D L algorithm. The strict
   --  solver below retains its original forward LDL' and rejection policy.
   procedure Solve_Compatible (D : in out Simulation; Result : out Status)
     with Global => null,
     Pre => Is_Ready (D) and then Array_Bounded (D.Scratch.Factor)
   ...
   begin
      ...
      for K in reverse 0 .. N - 1 loop
         pragma Loop_Invariant (Array_Bounded (D.Scratch.Factor));
         Pivot := SK.Clamp_Pivot (D.Scratch.Factor (K * N + K));
         if D.Scratch.Factor (K * N + K) < Min_Val and then D.First_Clamped < 0 then
            D.First_Clamped := K;
         end if;
         D.Scratch.Factor (K * N + K) := Pivot;
         Inverses (K) := SK.Reciprocal (Pivot);
         for I in reverse 0 .. K - 1 loop
            pragma Loop_Invariant (Array_Bounded (D.Scratch.Factor));
            Multiplier := SK.Scale (-D.Scratch.Factor (K * N + I), Inverses (K));
            for J in 0 .. I loop
               pragma Loop_Invariant (Array_Bounded (D.Scratch.Factor));
               Value := SK.Add_Product
                 (D.Scratch.Factor (I * N + J), D.Scratch.Factor (K * N + J), Multiplier);
               if not Within_Work (Value) then return; end if;
         ...
         for I in 0 .. K - 1 loop
            pragma Loop_Invariant (Array_Bounded (D.Scratch.Factor));
            Value := SK.Scale (D.Scratch.Factor (K * N + I), Inverses (K));
            if not Within_Work (Value) then return; end if;
            D.Scratch.Factor (K * N + I) := Value;
         end loop;
      end loop;
      ...
      for I in reverse 0 .. N - 1 loop
         ...
         if D.Scratch.Solution (I) /= 0.0 then
            for J in 0 .. I - 1 loop
               pragma Loop_Invariant (Array_Bounded (D.Scratch.Solution));
               Value := SK.Add_Product
                 (D.Scratch.Solution (J), D.Scratch.Factor (I * N + J), -D.Scratch.Solution (I));
               if not Within_Work (Value) then return; end if;
      ...
      for I in 0 .. N - 1 loop
         ...
         Sum := 0.0;
         for J in 0 .. I - 1 loop
            pragma Loop_Invariant (Within_Work (Sum));
            Value := SK.Add_Product (Sum, D.Scratch.Factor (I * N + J), D.Scratch.Solution (J));
            if not Within_Work (Value) then return; end if;
            Sum := Value;
         end loop;
         Value := D.Scratch.Solution (I) - Sum;
         if not Within_Work (Value) then return; end if;
         D.Scratch.Solution (I) := Value;
      end loop;
      Result := Success;
