...
private
   ...
   procedure Prove_Readiness_Decomposition (D : Simulation) with Ghost => Static, Global => null,
     Post => Is_Ready (D) = Original_Ready (D);
   ...
   procedure Invalidate (Cache : in out Cache_Flags) with Global => null,
     Post => not Cache.Pose_Valid and then not Cache.Jacobian_Valid and then not Cache.Mass_Valid
