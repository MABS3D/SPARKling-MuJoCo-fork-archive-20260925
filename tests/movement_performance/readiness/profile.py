...
def main():
    ...
    if not a.fixtures:
        ...
        path.write_text(text)
        path=smooth/'mj-data-pipeline.adb';text=path.read_text()
        text=text.replace('      --  Jv/Jw refer', '      Movement_Profile.Start (Jacobians);\n      --  Jv/Jw refer',1)
        text=text.replace('      D.Cache.Jacobian_Valid := True;', '      Movement_Profile.Stop (Jacobians);\n      D.Cache.Jacobian_Valid := True;',1)
        path.write_text(text)
