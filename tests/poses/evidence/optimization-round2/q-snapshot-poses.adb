package body MJ.Poses with SPARK_Mode is
   ...
   procedure Transform (R : out Vector_3; Position : Vector_3; Q : Quaternion; Point : Vector_3) is
      ...
      Source_Q : constant Quaternion := Q;
   begin
      --  The reference correction is zero for every finite scalar quaternion.
      if Source_Q (1) = 0.0 and then Source_Q (2) = 0.0 and then Source_Q (3) = 0.0 then
         declare
         ...
         begin
            ...
            pragma Assert (Static => R (0) = Model.Transformed_Component (Position, Source_Q, Point, 0));
            pragma Assert (Static => R (1) = Model.Transformed_Component (Position, Source_Q, Point, 1));
            pragma Assert (Static => R (2) = Model.Transformed_Component (Position, Source_Q, Point, 2));
         end;
      else
         ...
         declare
            T0 : constant Tier1_Real := Intermediate (Source_Q, Point, 0);
            T1 : constant Tier1_Real := Intermediate (Source_Q, Point, 1);
            T2 : constant Tier1_Real := Intermediate (Source_Q, Point, 2);
            D0 : constant Tier3_Real := Complete_Transform (Point (0), Position (0), Source_Q (2), Source_Q (3), T2, T1);
            D1 : constant Tier3_Real := Complete_Transform (Point (1), Position (1), Source_Q (3), Source_Q (1), T0, T2);
            D2 : constant Tier3_Real := Complete_Transform (Point (2), Position (2), Source_Q (1), Source_Q (2), T1, T0);
         begin
            R (0) := D0; R (1) := D1; R (2) := D2;
            pragma Assert (Static => R'Initialized);
            pragma Assert (Static => (for all I in Axis => R (I) in Tier3_Real));
