package body MJ.Poses with SPARK_Mode is
   ...
   procedure Prove_Zero_Point (Position : Vector_3; Q : Quaternion) with
     Ghost => Static, Global => null,
     Pre => MJ.BLAS.In_Tier0 (Position) and then MJ.Quaternions.In_Tier0 (Q),
     Post => (for all I in Axis =>
       Model.Transformed_Component (Position, Q, [0.0, 0.0, 0.0], I) = Position (I))
   is
   begin
      pragma Assert (MJ.Quaternions.Model.Rotation_Intermediate (Q, [0.0, 0.0, 0.0], 0) = 0.0);
      pragma Assert (MJ.Quaternions.Model.Rotation_Intermediate (Q, [0.0, 0.0, 0.0], 1) = 0.0);
      pragma Assert (MJ.Quaternions.Model.Rotation_Intermediate (Q, [0.0, 0.0, 0.0], 2) = 0.0);
      pragma Assert (Model.Transformed_Component (Position, Q, [0.0, 0.0, 0.0], 0) = Position (0));
      pragma Assert (Model.Transformed_Component (Position, Q, [0.0, 0.0, 0.0], 1) = Position (1));
      pragma Assert (Model.Transformed_Component (Position, Q, [0.0, 0.0, 0.0], 2) = Position (2));
   end Prove_Zero_Point;
   ...
   begin
      if Point (0) = 0.0 and then Point (1) = 0.0 and then Point (2) = 0.0 then
         Prove_Zero_Point (Position, Q);
         R := Position;
      ...
      elsif Q (1) = 0.0 and then Q (2) = 0.0 and then Q (3) = 0.0 then
         declare
