...
NAMES = {1:'compose',2:'compose_in_place',3:'inverse',4:'inverse_in_place',5:'transform',6:'transform_in_place',7:'quaternion_product_value'}
...
def main():
    ...
    assert len(set(patterns))==len(patterns) and set(patterns)<=set(range(10))
    ...
    report = {
        ...
        'fixture':{
            ...
            'checksum':'all written components outside both clocks',
            'timing_wrappers':'each operation has a separate noinline loop in both languages; kernels may inline',
            'in_place':'C pose wrappers snapshot inputs before upstream calls, matching the Ada interface',
        ...
        'build_flags':{
            ...
            'c':['-std=gnu11','-DmjUSEPLATFORMSIMD','-DNDEBUG'],
        },
