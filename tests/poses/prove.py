...
def main():
    ...
    parser.add_argument('--unit',choices=['mj-poses','mj-quaternions','mj-rotations'],default='mj-poses')
    unit=parser.parse_args().unit
    build_root=HERE/'build'/unit
    directory=build_root/'obj/proof/gnatprove'
    ...
    command=['gnatprove','-P',str(project),'-XPOSE_PROOF_ROOT='+str(build_root),'-u',unit+'.ads','--level=2',
    ...
    report.record_invocation(directory,unit,started,sources,command)
    result=report.main(['--unit',unit,'--mode','proof','--build-root',str(build_root),'--since',str(started),'-v'])
    if result:return result
    receipt={'started':started,'command':command,'source_sha256':sources,
             'project_sha256':project_hash,'report_sha256':hashlib.sha256((directory/(unit+'.spark')).read_bytes()).hexdigest()}
    (build_root/'proof-receipt.json').write_text(json.dumps(receipt,indent=2)+'\n')
    return 0
